...
const { kRootDir, kIsMacOS } = require("./constants");
...
const parseMacOSMemory = () => {
  try {
    ...
    const actualPageSize = pageSizeMatch
    ...
    const usedBytes = (active + wired + compressed + speculative) * actualPageSize;
...
const getSystemResources = ({ gatewayPid = null } = {}) => {
  ...
  const macOSMem = kIsMacOS ? parseMacOSMemory() : null;
  const mem = {
    usedBytes: cgroupMem?.usedBytes ?? macOSMem?.usedBytes ?? process.memoryUsage().rss,
    totalBytes: cgroupMem?.totalBytes ?? macOSMem?.totalBytes ?? os.totalmem(),
  };
